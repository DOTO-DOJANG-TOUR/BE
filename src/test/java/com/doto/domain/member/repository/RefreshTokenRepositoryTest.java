...
class RefreshTokenRepositoryTest extends AbstractRepositoryTest {
    ...
    void 토큰_해시로_조회할_수_있다() {
        ...
        refreshTokenRepository.save(RefreshToken.issue(member, "hashed-token", Instant.now().plusSeconds(60)));
    ...
    void 사용_가능한_토큰은_원자적으로_폐기되고_같은_토큰을_다시_폐기하면_0행이다() {
        ...
        RefreshToken token = refreshTokenRepository.save(
                RefreshToken.issue(member, "hashed-token", Instant.now().plusSeconds(60))
        );
    ...
    void 만료된_토큰은_폐기_대상에서_제외된다() {
        Member member = memberRepository.save(Member.register("홍길동"));
        RefreshToken token = refreshTokenRepository.save(
                RefreshToken.issue(member, "expired-token", Instant.now().minusSeconds(1))
        );
